...
/** Client of the catalog's signing endpoint, requesting {@code remote-presigning} results. */
public class RemoteSigningClient implements AutoCloseable {
  ...
  public static final String REMOTE_PRESIGNING = "remote-presigning";
  ...
  public URI preSign(String location, RemoteSignRequest request) {
    RemoteSignResponse response =
        restClient
            ...
            .post(
                ...
                headers(REMOTE_PRESIGNING),
                RemoteSigningClient::handleError);
    ...
    if (!response.headers().isEmpty()) {
      throw new UnsupportedOperationException(
          String.format(
              "Pre-signed URL for %s requires request headers %s, which are not supported",
              location, response.headers().keySet()));
    }

    return response.uri();
  }

  private static void handleError(ErrorResponse error) {
    if (error.code() == 406) {
      throw new UnsupportedOperationException(
          "Signing endpoint does not support " + REMOTE_PRESIGNING + ": " + error.message());
    }
